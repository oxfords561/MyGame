...
public class ResSvc : MonoSingleton<ResSvc>
{
    ...
    public void AsyncLoadScene(string sceneName, Action loaded)
    {
        ...
        prgCB = () =>
        {
            ...
            //(UIMgr.GetPanel(QAssetBundle.Loadingwnd_prefab.LOADINGWND) as LoadingWnd) .SetProgress(val);

            UIManager.Instance.HideUI(QAssetBundle.Loadingwnd_prefab.LOADINGWND);
            if (val == 1)
            {
                if (loaded != null)
                {
